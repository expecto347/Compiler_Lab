...
int main() {
  ...
  auto *arrayType = ArrayType::get(Int32Type, 10);  //定义一个10个元素的数组类型
  
  // main函数
  auto mainFun = Function::create(FunctionType::get(Int32Type, {}),
                                    "main", module); //创建main函数
  ...
  auto a0GEP = builder->create_gep(arrayAlloca, {CONST_INT(0), CONST_INT(0)});  //a[0]的地址
  builder->create_store(CONST_INT(10), a0GEP);                                  //a[0] = 10
  auto a0Load = builder->create_load(a0GEP);
  auto mul = builder->create_imul(a0Load, CONST_INT(2));                        //a[0] * 2
  auto a1GEP = builder->create_gep(arrayAlloca, {CONST_INT(0), CONST_INT(1)});  //a[1]的地址
  builder->create_store(mul, a1GEP);                                            //a[1] = a[0] * 2

  auto a1Load = builder->create_load(a1GEP);                                    //a[1]
  builder->create_ret(a1Load);                                                  //return a[1]
  std::cout << module->print();                                                 //输出生成的IR
  delete module;
  return 0;
}